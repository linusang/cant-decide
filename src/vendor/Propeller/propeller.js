...
(function(w) {
  ...
  p.updateAngleToMouse = function(newPoint) {
    ...
    var newMouseAngle =
      Math.atan2(newPoint.y - center.y, newPoint.x - center.x) + Math.PI;
  ...
  p.initOptions = function(options) {
    ...
    this.touchElement =
      document.querySelectorAll(options.touchElement)[0] || this.element;
    ...
    this.stepTransitionTime =
      options.stepTransitionTime || defaults.stepTransitionTime;
    this.stepTransitionEasing =
      options.stepTransitionEasing || defaults.stepTransitionEasing;
    ...
    this.lastAppliedAngle = this.virtualAngle = this._angle =
      options.angle || defaults.angle;
    this.minimalAngleChange =
      this.step !== defaults.step ? this.step : defaults.minimalAngleChange;
    this.rotateParentInstantly =
      options.rotateParentInstantly || defaults.rotateParentInstantly;
  };
  ...
  p.updateCSS = function() {
    this.element.style[Propeller.cssPrefix + "transform"] =
